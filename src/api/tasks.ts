...
/**
 * Fetches every task in a list. `showHidden` is required to see completed
 * tasks, which we need for the parent progress rollup even though we do not
...
export async function listEverything(): Promise<{ lists: GTaskList[]; tasks: WireTask[] }> {
  ...
  const perList = await Promise.all(lists.map((list) => listTasks(list.id)))
